...
export async function verifyTx(signature, amount) {
  ...
  if (!data.result) return { ok: false, reason: "tx not found" };
  ...
  if (!transfer) return { ok: false, reason: "wrong receiver" };

  const lamports = transfer.parsed.info.lamports;
  const expected = PRICE[amount];

  if (!expected) return { ok: false, reason: "invalid amount" };

  if (lamports < expected) {
    return { ok: false, reason: "not enough payment" };
  }
